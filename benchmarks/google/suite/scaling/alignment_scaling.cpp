...
// benchmark run would exhaust real memory — stdPool included, since
...
static void alignment_small_pool(benchmark::State& state) {
...
BENCHMARK(alignment_small_pool);

// stdPool counterpart of alignment_small_pool.
static void alignment_small_std(benchmark::State& state) {
    stdPool sSrc;
...
BENCHMARK(alignment_small_std);

// Measures allocate() at a 64-byte alignment.
static void alignment_medium_pool(benchmark::State& state) {
...
BENCHMARK(alignment_medium_pool);

// stdPool counterpart of alignment_medium_pool.
static void alignment_medium_std(benchmark::State& state) {
    stdPool sSrc;
    std::size_t sCounter = 0;
...
BENCHMARK(alignment_medium_std);

// Measures allocate() at a 4096-byte alignment.
static void alignment_large_pool(benchmark::State& state) {
...
BENCHMARK(alignment_large_pool);

// stdPool counterpart of alignment_large_pool.
static void alignment_large_std(benchmark::State& state) {
    stdPool sSrc;
    std::size_t sCounter = 0;
