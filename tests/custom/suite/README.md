...
- **Registration** — every test file ends with `REGISTER_TEST_SUITE();`,
  which registers the suite automatically at startup and assigns it a
  sequential id within its category (e.g. `C1`, `C2` for Concurrency; `U1`,
  `U2` for Unit) — there's no suite list to maintain by hand. This applies
  uniformly across every category above.
- **Test cases** — every case is a `static void <name>()` listed in the
  file's `run_tests()` via `RUN(<name>)`, and checks its results with
  `CHK()`.
