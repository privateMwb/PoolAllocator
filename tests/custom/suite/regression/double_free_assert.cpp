...
static void reuse_after_free_then_free() {
...
static void freeing_two_blocks_consecutively() {
    Pool<> pool(sizeof(void*), 4);
    void* a = pool.allocate();
...
static void many_sequential_frees_no_assert() {
...
static void run_tests() {
    RUN(reuse_after_free_then_free);
    RUN(freeing_two_blocks_consecutively);
    RUN(many_sequential_frees_no_assert);
}
