...
TEST(CtorPreconditionOrder, MinimumValidBlockCountSucceeds) {
    Pool<> pool(sizeof(void*), 1);
    EXPECT_EQ(pool.totalBlocks(), 1u);
...
// pointer).
TEST(CtorPreconditionOrder, MinimumValidBlockSizeSucceeds) {
    Pool<> pool(sizeof(void*), 4);
    EXPECT_GE(pool.blockStride(), sizeof(void*));
