...
`packaging/recipes/poolpro/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball extracts to `PoolAllocator-<version>`, not
  `PoolPro-<version>`** — the GitHub repo is `PoolAllocator`; the CMake
  project/library target inside it is `PoolPro`. `cd` into whatever
...
```bash
find $HOME/staging/lib/cmake/PoolPro -name "*.cmake"
```

Expect `PoolProConfig.cmake`, `PoolProConfigVersion.cmake`,
`PoolProTargets.cmake` (plus a per-config
`PoolProTargets-<config>.cmake`) under `lib/cmake/PoolPro/`.
...
target_link_libraries(test PRIVATE PoolPro::PoolPro)
EOF

cat > main.cpp << 'EOF'
#include <PoolPro/Pool.h>
int main() {
    rain::Pool<> pool(64, 16);
    return 0;
}
EOF
...
just default-constructs a `PoolPro::PoolAllocator` and returns, so success
...
```bash
conan create packaging/recipes/poolpro/all --version 1.0.0 \
...
`packaging/recipes/poolpro/all/test_package/` against the freshly-built
package — a `PoolPro linked and constructed successfully.` line in
