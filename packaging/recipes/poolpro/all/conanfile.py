...
class Conan(ConanFile):
    ...
    # edited here.
    name = "poolpro"
    cmake_name = "PoolPro"  # matches project()'s name in the top-level CMakeLists.txt
    version = "1.0.0"

    url = "https://github.com/privateMwb/PoolAllocator"
    description = "High-performance, header-only fixed-block pool allocator with O(1) allocate/deallocate/reset, backed by a hybrid free-list and bump-pointer watermark."
    topics = (
        "memory-allocator",
        "pool-allocator",
        "allocator",
    ...
    # header-library, not "library": there's no compiled src/PoolPro/*.cpp
