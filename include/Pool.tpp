...
template<bool EnableStats>
constexpr std::size_t Pool<EnableStats>::alignForward(std::size_t value, std::size_t alignment) noexcept {
...
void Pool<EnableStats>::initializeFreeList() noexcept {
	...
	for (std::size_t i = 1; i < blockCount_; ++i) {
		current->next  = reinterpret_cast<FreeNode*>(memory_ + i * stride_);
